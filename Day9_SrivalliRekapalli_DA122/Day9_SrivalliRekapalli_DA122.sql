...
·        Test the trigger by updating the product price by 10% to any one product_id.
	...
	-- Check current price
SELECT product_id, product_name, unit_price FROM products WHERE product_id = 101;
...
-- Check the audit log
SELECT * FROM product_price_audit WHERE product_id = 101 ORDER BY change_date DESC;
...
BEGIN
    ...
    -- Raise a notice with task details
    RAISE NOTICE 'Task "%" assigned to employee %. Total tasks: %',
        p_task_name, p_employee_id, p_task_count;
END;
$$;
...
-- Second call with INOUT usage
DO $$
DECLARE
    task_total INT := 0;
BEGIN
    CALL assign_task(1, 'Prepare Presentation', task_total);
...
-- Verify the Results
-- View tasks assigned to employee_id = 1
