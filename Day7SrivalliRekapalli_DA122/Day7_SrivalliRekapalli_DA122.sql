...
SELECT 
    ...
    ROUND((freight - LAG(freight) OVER (PARTITION BY customer_id ORDER BY order_date))::numeric,2) AS compare_from_prev,
    ROUND((LEAD(freight) OVER (PARTITION BY customer_id ORDER BY order_date) - freight)::numeric,2) AS compare_to_next
FROM 
    orders
ORDER BY 
...
WITH PriceCategories AS (
    SELECT 
        ...
        CASE 
            ...
            WHEN unit_price < 50 THEN 'Medium Price'
            ELSE 'High Price'
        END AS price_category, unit_price
    FROM products
)
SELECT 
    price_category,
